...
Before migration, stop or drain writers as required by the migration, take an encrypted backup, record current migration/schema state, and apply each missing migration in numeric order. Where a statement uses `IF NOT EXISTS` (including migration 013), that additive DDL is retry-safe, but this is not a blanket idempotency claim for every historical migration and it does not verify an existing object's definition; record success and run the explicit schema checks. Existing-volume prerequisites are in `README.md`; Docker init mounts do not migrate existing databases. Fail closed if required tables/columns are absent.
