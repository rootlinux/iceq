...
| `(cd backend && go test ./edge-identity ./shared/middleware ./auth-service/handlers ./key-service/handlers)` | Exit 0. Includes signer HMAC/rotation/spoof/no-log/input validation and refresh-reuse concurrent/fault-injection regressions. |
...
| `./deploy/scripts/check-edge-identity.test.sh` | Exit 0. Static contract confirms Caddy strips client trust headers, only the signer receives `remote_host`, application upstreams receive only the copied digest, and the signer has no published port. |
