...
test("encryptFileBlob returns opaque ciphertext and a decryptable manifest", async () => {
  ...
  const encrypted = await encryptFileBlob(plaintext, "notes.txt");
  ...
  const decrypted = await decryptFileBlob(encrypted.encryptedBlob, encrypted.manifest);
...
test("encryptFileBlob does not preserve unsafe display paths", async () => {
  ...
  const encrypted = await encryptFileBlob(plaintext, "../private/secret.txt");
