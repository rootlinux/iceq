...
export async function ensureGroupSender(ns:CryptoNamespace,groupId:string,epoch:number,selfUin:number,memberUins:number[],sendDistribution:(uin:number, plaintext:Uint8Array, distribution:SenderKeyDistribution)=>Promise<void>):Promise<SenderState> {
  return withSenderLock(`${ns.uin}:${ns.deviceId}:${groupId}:${epoch}:${selfUin}`,()=>ensureGroupSenderCAS(ns,groupId,epoch,selfUin,memberUins,sendDistribution));
}

export async function ensureGroupSenderCAS(ns:CryptoNamespace,groupId:string,epoch:number,selfUin:number,memberUins:number[],sendDistribution:(uin:number, plaintext:Uint8Array, distribution:SenderKeyDistribution)=>Promise<void>,options:SenderDeliveryOptions={}):Promise<SenderState>{
  ...
  let stored=await loadGroupCryptoState(ns,groupId,epoch,selfUin,"sender");
  if(!stored){const made=await createSenderState(groupId,epoch,selfUin);const local:LocalStored={sender:made.state,distributed_to:[],revision:0};const record={version:1 as const,kind:"sender" as const,group_id:groupId,epoch,sender_uin:selfUin,updated_at:Date.now(),state:local};if(await compareAndSwapSenderState(ns,record,null))stored=record;else stored=await loadGroupCryptoState(ns,groupId,epoch,selfUin,"sender");}
  ...
  if(!await loadGroupCryptoState(ns,groupId,epoch,selfUin,"receiver",distribution.distribution_id))await saveGroupCryptoState(ns,{version:1,kind:"receiver",group_id:groupId,epoch,sender_uin:selfUin,updated_at:Date.now(),state:createReceiverState(distribution)});
  for(const uin of [...new Set(memberUins)].filter(u=>u!==selfUin).sort((a,b)=>a-b))await ensureRecipientDelivery(ns,groupId,epoch,selfUin,uin,sendDistribution,{now,sleep,leaseMs,tokenFactory});
  const latest=await loadGroupCryptoState(ns,groupId,epoch,selfUin,"sender");if(!latest)throw new Error("sender state disappeared");return (latest.state as LocalStored).sender;
}
async function ensureRecipientDelivery(ns:CryptoNamespace,groupId:string,epoch:number,selfUin:number,uin:number,sendDistribution:(uin:number,plaintext:Uint8Array,distribution:SenderKeyDistribution)=>Promise<void>,options:{now:()=>number;sleep:(ms:number)=>Promise<void>;leaseMs:number;tokenFactory:()=>string}):Promise<void>{
  for(;;){
    const stored=await loadGroupCryptoState(ns,groupId,epoch,selfUin,"sender");if(!stored)throw new Error("sender state disappeared");const local=stored.state as LocalStored;const key=String(uin);const delivery=local.deliveries?.[key];
    ...
    if(!await compareAndSwapSenderState(ns,{...stored,updated_at:options.now(),state:next},revision))continue;
    try{await sendDistribution(uin,new TextEncoder().encode(JSON.stringify({kind:GROUP_DISTRIBUTION_KIND,distribution:immutableDistribution})),immutableDistribution);}catch(error){await clearFailedDelivery(ns,groupId,epoch,selfUin,uin,token,options.now);throw error;}
    await completeRecipientDelivery(ns,groupId,epoch,selfUin,uin,token,immutableDistribution,options.now);return;
  }
}
async function clearFailedDelivery(ns:CryptoNamespace,groupId:string,epoch:number,selfUin:number,uin:number,token:string,now:()=>number):Promise<void>{for(let attempt=0;attempt<16;attempt++){const stored=await loadGroupCryptoState(ns,groupId,epoch,selfUin,"sender");if(!stored)return;const local=stored.state as LocalStored;const delivery=local.deliveries?.[String(uin)];if(delivery?.status!=="pending"||delivery.token!==token)return;const deliveries={...(local.deliveries??{})};delete deliveries[String(uin)];const revision=local.revision??0;if(await compareAndSwapSenderState(ns,{...stored,updated_at:now(),state:{...local,deliveries,revision:revision+1}},revision))return;}}
async function completeRecipientDelivery(ns:CryptoNamespace,groupId:string,epoch:number,selfUin:number,uin:number,_token:string,delivered:SenderKeyDistribution,now:()=>number):Promise<void>{for(let attempt=0;attempt<16;attempt++){const stored=await loadGroupCryptoState(ns,groupId,epoch,selfUin,"sender");if(!stored)throw new Error("sender state disappeared");const local=stored.state as LocalStored;const delivery=local.deliveries?.[String(uin)];if(delivery&&!sameDistribution(delivery.distribution,delivered))throw new Error("sender distribution context changed during delivery");if(delivery?.status==="completed")return;const revision=local.revision??0;const completed:RecipientDelivery={status:"completed",completed_at:now(),attempts:delivery?.attempts??1,distribution:delivered};const next={...local,distributed_to:[...new Set([...local.distributed_to,uin])],deliveries:{...(local.deliveries??{}),[String(uin)]:completed},revision:revision+1};if(await compareAndSwapSenderState(ns,{...stored,updated_at:now(),state:next},revision))return;}throw new Error("sender delivery completion changed concurrently");}
...
export async function sealGroupContent(ns:CryptoNamespace,state:SenderState,content:GroupContent):Promise<SenderKeyCiphertext> {
  const lockKey=`${ns.uin}:${ns.deviceId}:${state.group_id}:${state.epoch}:${state.sender_uin}`;
  return withSenderLock(lockKey,()=>sealGroupContentCAS(ns,state,content));
}
export async function sealGroupContentCAS(ns:CryptoNamespace,state:SenderState,content:GroupContent):Promise<SenderKeyCiphertext>{
  for(let attempt=0;attempt<16;attempt++){
    let stored=await loadGroupCryptoState(ns,state.group_id,state.epoch,state.sender_uin,"sender");
    if(!stored){const initial:LocalStored={sender:state,distributed_to:[],revision:0};const created={version:1 as const,kind:"sender" as const,group_id:state.group_id,epoch:state.epoch,sender_uin:state.sender_uin,updated_at:Date.now(),state:initial};if(!await compareAndSwapSenderState(ns,created,null))continue;stored=created;}
    ...
    if(await compareAndSwapSenderState(ns,{...stored,updated_at:Date.now(),state:next},revision))return result.envelope;
  }
  throw new Error("sender state changed concurrently");
}
export async function installSenderDistribution(ns:CryptoNamespace,senderUin:number,value:unknown,currentEpoch:number,currentMembers:number[]):Promise<boolean> {
  ...
  if(await loadGroupCryptoState(ns,d.group_id,d.epoch,d.sender_uin,"receiver",d.distribution_id))return true;
  const receiver=createReceiverState(d);
  await saveGroupCryptoState(ns,{version:1,kind:"receiver",group_id:d.group_id,epoch:d.epoch,sender_uin:d.sender_uin,updated_at:Date.now(),state:receiver}); return true;
}
export async function processDirectControlMessage(ns:CryptoNamespace,senderUin:number,plaintext:Uint8Array,resolveGroup:(groupId:string)=>Promise<{epoch:number;members:number[]}>):Promise<boolean>{
  ...
  await installSenderDistribution(ns,senderUin,value,roster.epoch,roster.members);
  return true;
}
export async function hydrateSenderKeyInbox(ns:CryptoNamespace,groupId:string,currentEpoch:number,currentMembers:number[],fetchInbox:()=>Promise<OpaqueSenderKeyInboxItem[]>,decryptPairwise:(senderUin:number,ciphertext:string,msgType:"prekey_message"|"signal_message")=>Promise<Uint8Array>):Promise<number> {
  let installed=0;
  for(const item of await fetchInbox()) {
    ...
    const existing=item.distribution_id?await loadGroupCryptoState(ns,groupId,itemEpoch,item.sender_uin,"receiver",item.distribution_id):null;
    ...
    if(itemEpoch===currentEpoch){if(await installSenderDistribution(ns,item.sender_uin,value,currentEpoch,currentMembers))installed++;}
    else {
      const d=value.distribution as unknown as SenderKeyDistribution;
      ...
      await saveGroupCryptoState(ns,{version:1,kind:"receiver",group_id:groupId,epoch:itemEpoch,sender_uin:item.sender_uin,created_at:retiredAt,updated_at:Date.now(),state:createReceiverState(d)});installed++;
    }
  }
  return installed;
}
export async function openGroupContent(ns:CryptoNamespace,envelope:SenderKeyCiphertext,currentEpoch:number,currentMembers:number[],allowObsoleteDecrypt=false,now=Date.now(),expected?:ExpectedGroupRouting):Promise<GroupContent> {
  ...
  await pruneAuthenticatedGroupContent(ns,envelope.group_id,now);
  if(allowObsoleteDecrypt){const cached=await loadAuthenticatedGroupContent(ns,cacheKey,envelope.group_id,envelope.epoch,now);if(cached){if(!isValidGroupContent(cached))throw new Error("invalid cached group content");return cached;}}
  const stored=await loadGroupCryptoState(ns,envelope.group_id,envelope.epoch,envelope.sender_uin,"receiver",envelope.distribution_id);
  ...
  await saveGroupCryptoState(ns,{...stored,updated_at:Date.now(),state:receiver});
  const parsed:unknown=JSON.parse(new TextDecoder().decode(plaintext));
  if(!isValidGroupContent(parsed))throw new Error("invalid encrypted group content");
  await saveAuthenticatedGroupContent(ns,cacheKey,envelope.group_id,envelope.epoch,parsed,now);
