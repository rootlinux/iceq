...
grep -Fq 'npm audit --audit-level=moderate' "$workflow"
grep -Fq 'npm audit --audit-level=moderate --omit=dev' "$workflow"
grep -Fq './deploy/scripts/check-clearnet-compose-command.test.sh' "$workflow"
grep -Fq './deploy/scripts/check-clearnet-compose.sh' "$workflow"
